...
def main(segment, config):
    ...
    try:
        ...
        required_psd_values = psd_values([5], raw_trace, inv)
        psd5sec = required_psd_values[0]

        if np.isnan(psd5sec):
            raise ValueError('PSD(5) is NaN')
        anomaly_score = -if_supervised_model.score_samples([[psd5sec]])[0]

        # sn_wdw = config['sn_windows']
        # wlen_sec = sn_wdw['signal_window']
        # atime = utcdatetime(segment.arrival_time) + sn_wdw['arrival_time_shift']
        # sig_wdw = raw_trace.slice(atime, None)

        # bandpass the trace, according to the event magnitude.
        # This modifies the segment.stream() permanently:
        trace = _bandpass_remresp(segment, config, raw_trace, inv)    
        ...
        ret['channel_id'] = segment.channel.id
        ret['channel_code'] = segment.channel.channel
        ...
        ret['psd@5sec'] = psd5sec
        ret['predicted_anomaly_score'] = anomaly_score
